...
function App() {
  ...
  return (
    <div className="clastar-site">
      ...
      {menuOpen && <div className="mobile-menu"><button onClick={() => go('products')}>Products</button><button onClick={() => go('portfolio')}>Portfolio</button><button onClick={() => go('work')}>Work</button><button onClick={() => go('technology')}>Technology</button><button onClick={() => go('company')}>Company</button><button onClick={() => go('contact')}>Contact</button></div>}
      ...
      <main>
        ...
        <section className="games-section section-pad"><div className="container"><div className="section-head"><div><p className="section-kicker">10 / INTERACTIVE</p><h2>Interactive <span>experiences.</span></h2></div><p>Games and experiments are presented alongside the company products they belong to.</p></div><div className="game-strip">{games.map(([name, type], i) => <div className="game-card" key={name}><span>0{i + 1}</span><div><small>{type}</small><h3>{name}</h3></div><span className="complete">COMPLETE</span></div>)}</div></div></section>

        <section id="work" className="work-section section-pad"><div className="container"><div className="section-head"><div><p className="section-kicker">11 / SELECTED WORK</p><h2>Built for <span>business.</span></h2></div><p>Web experiences and digital systems created for companies and organizations.</p></div><div className="work-grid">{work.map(([name, type], i) => <a href="#portfolio" key={name} className="work-card"><div className="work-visual"><div className="work-window"><div className="window-bar"><i /><i /><i /></div><div className="window-lines"><b /><b /><b /></div><div className="window-block" /><div className="window-glow">0{i + 1}</div></div></div><div className="work-label"><div><span>{type}</span><h3>{name}</h3></div><ArrowUpRight size={19} /></div></a>)}</div></div></section>
        ...
        <section id="company" className="company-section section-pad"><div className="container company-layout"><div><p className="section-kicker">13 / COMPANY</p><h2>One hub.<br /><span>Many possibilities.</span></h2></div><div className="company-copy"><p>Clastar Hub is a technology company building products and digital infrastructure across education, finance, productivity, marketplaces, business websites and interactive experiences.</p><p>Every project in the portfolio is treated as part of the company ecosystem, with its own product story, visual evidence and end-product path.</p><div className="company-mark"><Cpu size={22} /> CLASTAR HUB</div></div></div></section>

        <section id="contact" className="contact-section"><div className="contact-glow" /><div className="container contact-inner"><p className="section-kicker">14 / CONTACT</p><h2>Let’s build<br /><span>what’s next.</span></h2><p>Have an idea, a product or a problem worth solving? Let’s talk.</p><a href="mailto:hello@clastarhub.com" className="primary-cta">Start a conversation <ArrowUpRight size={18} /></a></div></section>
      </main>
      ...
      <AnimatePresence>{selectedProduct && <motion.div className="project-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedProduct(null)}>
        <motion.div className="project-modal" initial={{ y: 40, opacity: 0, scale: .97 }} animate={{ y: 0, opacity: 1, scale: 1 }} exit={{ y: 20, opacity: 0, scale: .98 }} onClick={(e) => e.stopPropagation()}>
          ...
          {selectedProduct.href.startsWith('http') ? <a className="primary-cta" href={selectedProduct.href} target="_blank" rel="noreferrer">Visit live product <ArrowUpRight size={18} /></a> : <button className="primary-cta" onClick={() => { setSelectedProduct(null); go('portfolio'); }}>Explore product case study <ArrowUpRight size={18} /></button>}
        </motion.div>
