...
export default function Navbar() {
  ...
  useEffect(() => { document.body.classList.toggle('dark', dark); localStorage.setItem('clastar-theme', dark ? 'dark' : 'light'); }, [dark]);
  ...
  return <nav className="glass-nav">
    <div className="nav-inner">
      ...
      <div className="nav-links">{links.map(([label,id]) => <button key={label} onClick={() => scrollTo(id)}>{label}</button>)}</div>
      <button className="theme-button" onClick={() => setDark(v => !v)} aria-label="Toggle theme" title="Toggle light/dark mode">{dark ? <Sun size={17}/> : <Moon size={17}/>}</button>
